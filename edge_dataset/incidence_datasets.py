...
# -----------------------------------------------------------------------------
# 1. BaseGraph Class (Reference Code와 100% 동일한 구조 유지)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. load_dataset (Reference의 ppi_bp branch 로직을 그대로 사용)
# -----------------------------------------------------------------------------
def load_dataset(name: str):
    """
    DocRED, VisualGenome 등을 로드. 
    reference code의 'ppi_bp' 브랜치 로직을 그대로 사용하며,
    경로만 ./edge_dataset/ 으로 변경하고 .pth 파일도 텍스트로 읽음.
    """

    # DocRED, VisualGenome, Connectome 만 처리
    if name in ["DocRED", "VisualGenome", "Connectome"]:
        ...
        # (Reference와 동일 로직: 텍스트 파일을 라인별로 파싱)
        # ---------------------------------------------------------
        def read_subgraphs(sub_f, split=True):
            ...
            # ★ 핵심 수정: .pth 파일이라도 텍스트 모드('r')로 읽습니다.
            with open(sub_f, 'r') as fin:
                subgraph_idx = 0
                for line in fin:
                    # Line Format: "Node-Node-Node \t Label \t Split"
                    parts = line.strip().split("\t")
                    if len(parts) < 3: continue # 빈 줄 등 방지
                    ...
                    if len(nodes) != 0:
                        ...
                        # Label Mapping (String/Int -> Index)
                        for lab in l:
        ...
        # ---------------------------------------------------------
        # Caching Logic (Reference와 동일하게 .pt 파일 존재 시 로드, 없으면 생성)
        # ---------------------------------------------------------
        if os.path.exists(f"{base_path}/train_sub_G.pt"):
            train_sub_G = torch.load(f"{base_path}/train_sub_G.pt")
            train_sub_G_label = torch.load(f"{base_path}/train_sub_G_label.pt")
            val_sub_G = torch.load(f"{base_path}/val_sub_G.pt")
            val_sub_G_label = torch.load(f"{base_path}/val_sub_G_label.pt")
            test_sub_G = torch.load(f"{base_path}/test_sub_G.pt")
            test_sub_G_label = torch.load(f"{base_path}/test_sub_G_label.pt")
        else:
            # ★ 텍스트 파일(incidence_subgraph.pth)을 읽어서 처리
            train_sub_G, train_sub_G_label, val_sub_G, val_sub_G_label, test_sub_G, test_sub_G_label = read_subgraphs(
                f"{base_path}/incidence_subgraph.pth")
            
            # Cache 저장
            torch.save(train_sub_G, f"{base_path}/train_sub_G.pt")
            torch.save(train_sub_G_label, f"{base_path}/train_sub_G_label.pt")
            torch.save(val_sub_G, f"{base_path}/val_sub_G.pt")
            torch.save(val_sub_G_label, f"{base_path}/val_sub_G_label.pt")
            torch.save(test_sub_G, f"{base_path}/test_sub_G.pt")
            torch.save(test_sub_G_label, f"{base_path}/test_sub_G_label.pt")

        # ---------------------------------------------------------
        # Construct Data Objects (Mask, Label, Pos, Edge)
        # ---------------------------------------------------------
        mask = torch.cat(
        ...
        # ★ pad_sequence 적용 (List -> Tensor 변환)
        pos = pad_sequence(
            [torch.tensor(i) for i in train_sub_G + val_sub_G + test_sub_G],
            batch_first=True,
            padding_value=-1)
        
        # Edge List 로드 (NetworkX 사용)
        rawedge = nx.read_edgelist(f"{base_path}/incidence_edge_list.txt", nodetype=int).edges
        ...
        raise NotImplementedError("Only DocRED and VisualGenome are supported in this file.")