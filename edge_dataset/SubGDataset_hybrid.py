...
def _stack_labels(y_list, ref_y):
    """
    Preserve dtype/shape of labels:
      - if labels are tensors: stack (handles scalar, vector, multi-hot, multi-label)
      - else: make tensor with ref_y dtype if possible
    """
    if len(y_list) == 0:
        return torch.empty((0,), dtype=ref_y.dtype if torch.is_tensor(ref_y) else torch.float)

    y0 = y_list[0]
    if torch.is_tensor(y0):
        # make sure all are tensors on same device (caller moves dataset to device later)
...
class GDataset:
    ...
    def sample_pos_comp_test(self, m, M, views=1, device=0, row=None, col=None, dataset="ppi_bp"):
        ...
        # ★ 기존처럼 dataset 이름으로 dtype/shape를 강제하지 말고, 원본 y 그대로 유지
        self.y = _stack_labels(y, self.y)
